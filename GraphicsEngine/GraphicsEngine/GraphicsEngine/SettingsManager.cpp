...
void SettingsManager::CreateFile(const wstring& filename)
{
	...
	// Create root node:
	auto settingsNode = document.allocate_node(node_type::node_element, TAG_SETTINGS);
...
void SettingsManager::AddAdaptersInfo(rapidxml::xml_document<wchar_t>* document, rapidxml::xml_node<wchar_t>* parent)
{
	...
	auto videoCardsNode = document->allocate_node(node_type::node_element, TAG_VIDEO_CARDS);
	...
	auto defaultVideoCardIndexNode = document->allocate_node(node_type::node_element, TAG_DEFAULT_VIDEO_CARD_INDEX, AllocateValue(document, m_adapterIndex));
...
void SettingsManager::AddAdapterInfo(xml_document<wchar_t>* document, rapidxml::xml_node<wchar_t>* parent, UINT adapterIndex, const DXGI_ADAPTER_DESC1& adapterDesc) const
{
	...
	auto videoCardNode = document->allocate_node(node_type::node_element, TAG_VIDEO_CARD);
	auto indexNode = document->allocate_node(node_type::node_element, TAG_INDEX, AllocateValue(document, adapterIndex));
	auto descriptionNode = document->allocate_node(node_type::node_element, TAG_DESCRIPTION, document->allocate_string(adapterDesc.Description));
	auto dedicatedVideoMemoryNode = document->allocate_node(node_type::node_element, TAG_DEDICATED_VIDEO_MEMORY, AllocateValue(document, adapterDesc.DedicatedVideoMemory));
