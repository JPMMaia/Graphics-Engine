...
void SoundManager::Play2DSound(const string& name, float volumePercentage)
{
	...
	ThrowIfFailed(secondaryBuffer->SetCurrentPosition(0));

	// Set volume:
	{
		// Calculate volume. Map from range [0, 1] to [-10.000, 0]:
		auto volume = static_cast<long>(std::round((volumePercentage - 1.0f) * (-DSBVOLUME_MIN)));

		// Set volume of the buffer:
		ThrowIfFailed(secondaryBuffer->SetVolume(volume));
	}

	// Play the contents of the secondary sound buffer:
	ThrowIfFailed(secondaryBuffer->Play(0, 0, 0));
}

void SoundManager::InitializeDirectSound(HWND windowHandle)
{
	// Create Direct Sound:
	ThrowIfFailed(DirectSoundCreate8(nullptr, m_directSound.GetAddressOf(), nullptr));

	// Set the cooperative level to priority so the format of the primary sound buffer can be modified:
	ThrowIfFailed(m_directSound->SetCooperativeLevel(windowHandle, DSSCL_PRIORITY));
	...
	ThrowIfFailed(m_directSound->CreateSoundBuffer(&bufferDescription, m_primaryBuffer.GetAddressOf(), nullptr));
	...
	ThrowIfFailed(m_primaryBuffer->SetFormat(&waveFormat));
}
...
void SoundManager::LoadWaveFile(const std::wstring& filename, IDirectSoundBuffer8** secondaryBuffer) const
{
	...
	// Create secondary buffer:
	{
		...
		ThrowIfFailed(m_directSound->CreateSoundBuffer(&bufferDescription, temporaryBuffer.GetAddressOf(), nullptr));

		// Test the buffer format against the direct sound 8 interface and create the secondary buffer:
		temporaryBuffer.Get()->QueryInterface(IID_IDirectSoundBuffer8, reinterpret_cast<void**>(secondaryBuffer));
	}
	...
	// Copy data from waveData to secondary buffer:
	{
		...
		ThrowIfFailed((*secondaryBuffer)->Lock(0, waveFileHeader.DataSize, reinterpret_cast<void**>(&pBuffer), &bufferSize, nullptr, nullptr, 0));

		// Copy the wave data into the buffer:
		CopyMemory(pBuffer, &waveData[0], waveFileHeader.DataSize);

		// Unlock the secondary buffer after the data has been written to it:
		ThrowIfFailed((*secondaryBuffer)->Unlock(static_cast<void*>(pBuffer), bufferSize, nullptr, 0));
	}
}
