...
namespace GraphicsEngine
{
	...
	public:
		Buffer();
		Buffer(ID3D11Device* d3dDevice, uint32_t bufferTypeSize);
		...
		void Update(ID3D11DeviceContext1* d3dDeviceContext, const void* bufferData) const;
	...
	template <D3D11_BIND_FLAG BIND_FLAG, D3D11_USAGE USAGE_FLAG, uint32_t CPU_ACCESS_FLAG>
	Buffer<BIND_FLAG, USAGE_FLAG, CPU_ACCESS_FLAG>::Buffer(ID3D11Device* d3dDevice, uint32_t bufferTypeSize) :
		m_stride(bufferTypeSize)
	{
		// Create an empty buffer with the specified size:
		CreateBuffer(d3dDevice, bufferTypeSize, nullptr);
	}
		...
		m_stride(sizeof(BufferType))
	{
		...
		CreateBuffer(d3dDevice, sizeof(BufferType) * initialData.size(), &subresourceData);
	}
	...
	template <D3D11_BIND_FLAG BIND_FLAG, D3D11_USAGE USAGE_FLAG, uint32_t CPU_ACCESS_FLAG>
	template <typename>
	void Buffer<BIND_FLAG, USAGE_FLAG, CPU_ACCESS_FLAG>::Update(ID3D11DeviceContext1* d3dDeviceContext, const void* bufferData) const
	{
	...
	template <D3D11_BIND_FLAG BIND_FLAG, D3D11_USAGE USAGE_FLAG, uint32_t CPU_ACCESS_FLAG>
	uint32_t Buffer<BIND_FLAG, USAGE_FLAG, CPU_ACCESS_FLAG>::GetStride() const
	{
		return m_stride;
	}
